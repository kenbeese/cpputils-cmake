...
(defun cppcm-get-root-source-dir (d)
  (let (rlt)
    (setq rlt (cppcm-query-var-from-last-matched-line (concat d "CMakeCache.txt") "CMAKE_HOME_DIRECTORY\:INTERNAL\=\\(.*\\)"))
...
(defun cppcm-create-one-makefile (root-src-dir build-dir cm tgt mk)
  (let (flag-make
    ...
    (when exe-full-path
      ...
      (when (file-exists-p flag-make)
        (when (setq queried-c-flags (cppcm-query-match-line flag-make "\s*\\(CX\\{0,2\\}_FLAGS\\)\s*=\s*\\(.*\\)"))
        ...
        (with-temp-file mk
          (insert (cppcm-flymake-makefile-str build-dir flag-make))))
    )))

(defun cppcm-read-compiler (build-dir flag-make)
  (cppcm-query-var-from-last-matched-line (concat build-dir "CMakeCache.txt")
   (concat "CMAKE_" (cppcm-read-lang flag-make) "_COMPILER:FILEPATH=\\(.*\\)")))

(defun cppcm-read-lang (flag-make)
  (let (v (lines (cppcm-readlines flag-make)))
    (catch 'brk
      (dolist (l lines)
        (when (string-match "# compile \\(\\w+\\) with .*" l)
          (setq v (match-string 1 l))
          (throw 'brk t))))
    v)
  )

(defun cppcm-flymake-makefile-str (build-dir flag-make)
  (let ((lang (cppcm-read-lang flag-make))
        (compiler (cppcm-read-compiler build-dir flag-make)))
