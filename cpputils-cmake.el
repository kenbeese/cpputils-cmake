...
(defun cppcm-get-source-dir (d)
    (cppcm-query-var (concat d "CMakeCache.txt") "CMAKE_HOME_DIRECTORY\:INTERNAL\=\\(.*\\)")
    )
    ...
    (when exe-full-path
      ...
      (when (and (file-exists-p flag-make) ml)
        ...
        (with-temp-file mk
          ...
          (insert (cppcm-flymake-makefile-str
                   (cadr lang-compiler) (car lang-compiler) flag-make)))
        )
      )
    )
  )

(defun cppcm-read-lang-compiler (flag-make)
  (let (v (lines (cppcm-readlines flag-make)))
    (catch 'brk
      (dolist (l lines)
        (when (string-match "# compile \\(\\w+\\) with \\(.+\\)" l)
          (setq v (list (match-string 1 l) (match-string 2 l)))
          (throw 'brk t)
          )
        )
      )
...
(defun cppcm-flymake-makefile-str (compiler lang flag-make)
  (concat "# Generated by " cppcm-prog ".\n"
          ...
          "_FLAGS} ${"
          lang
          "_DEFINES} -S -fsyntax-only ${CHK_SOURCES}"
