  ...
  (let (flag-make
    ...
    (when exe-full-path
      ...
      (when (and (file-exists-p flag-make)
        ...
        (with-temp-file mk
          (insert (cppcm-flymake-makefile-str flag-make))))
    ))
...
(defun cppcm-flymake-makefile-str (flag-make)
  (let* ((lang-compiler (cppcm-read-lang-compiler))
         (lang (car lang-compiler))
         (compiler (cadr lang-compiler)))
